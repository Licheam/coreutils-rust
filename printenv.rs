...
pub const PRINTENV_FAILURE: C2RustUnnamed_0 = 2;
#[no_mangle]
pub unsafe extern "C" fn usage(mut status: libc::c_int) -> ! {
    if status != 0 as libc::c_int {
        ...
        printf(
            ...
            b"printenv\0" as *const u8 as *const libc::c_char,
        );
        emit_ancillary_info(b"printenv\0" as *const u8 as *const libc::c_char);
    }
    exit(status);
}
#[inline]
...
unsafe fn main_0(
    ...
    initialize_exit_failure(PRINTENV_FAILURE as libc::c_int);
    atexit(Some(close_stdout as unsafe extern "C" fn() -> ()));
    loop {
        optc = getopt_long(
            argc,
            argv,
            b"+iu:0\0" as *const u8 as *const libc::c_char,
            longopts.as_ptr(),
            0 as *mut libc::c_int,
        );
        ...
        match optc {
            ...
            _ => {
                usage(PRINTENV_FAILURE as libc::c_int);
            }
        }
    }
    if optind >= argc {
        ...
        while i < argc {
            ...
            if (strchr(*argv.offset(i as isize), '=' as i32)).is_null() {
                ...
                while !(*env).is_null() {
                    ep = *env;
                    ap = *argv.offset(i as isize);
                    while *ep as libc::c_int != '\0' as i32
                        && *ap as libc::c_int != '\0' as i32
                        && {
                            let fresh0 = ep;
                            ep = ep.offset(1);
                            let fresh1 = ap;
