...
pub unsafe extern "C" fn usage(mut status: libc::c_int) -> ! {
    if status != 0 as libc::c_int {
        ...
        emit_ancillary_info(b"logname\0" as *const u8 as *const libc::c_char);
    }
    exit(status);
}
#[inline]
...
unsafe fn main_0(
    ...
    parse_gnu_standard_options_only(
        ...
        b"FIXME: unknown\0" as *const u8 as *const libc::c_char,
    ...
    if cp.is_null() {
        if ::core::mem::size_of::<C2RustUnnamed>() as libc::c_ulong != 0 {
            error(
                1 as libc::c_int,
                0 as libc::c_int,
                gettext(b"no login name\0" as *const u8 as *const libc::c_char),
            );
