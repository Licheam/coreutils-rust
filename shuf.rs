...
pub const RANDOM_SOURCE_OPTION: C2RustUnnamed_2 = 128;
#[derive(Copy, Clone)]
#[repr(C)]
...
unsafe extern "C" fn input_from_argv(
    ...
    while i < n_operands {
        ...
        let ref mut fresh0 = *operand.offset(i as isize);
        *fresh0 = p;
        p = p1;
        let fresh1 = p;
        p = p.offset(1);
    ...
    let ref mut fresh2 = *operand.offset(n_operands as isize);
    *fresh2 = p;
}
...
unsafe extern "C" fn read_input(
            ...
            as libc::c_int != eolbyte as libc::c_int
    {
        ...
        *buf.offset(fresh4 as isize) = eolbyte;
    }
    ...
    *pline = line;
    p = buf;
    let ref mut fresh5 = *line.offset(0 as libc::c_int as isize);
    *fresh5 = p;
    let mut i: size_t = 1 as libc::c_int as size_t;
    while i <= n_lines {
        p = next_line(p, eolbyte);
        let ref mut fresh6 = *line.offset(i as isize);
        *fresh6 = p;
        i = i.wrapping_add(1);
        i;
    }
...
unsafe fn main_0(
    ...
    loop {
        ...
        match optc {
            ...
            105 => {
                let mut p: *mut libc::c_char = strchr(optarg, '-' as i32);
                let mut hi_optarg: *const libc::c_char = optarg;
                let mut invalid: bool = p.is_null();
                if input_range {
                ...
                if !p.is_null() {
                    *p = '\0' as i32 as libc::c_char;
                    lo_input = xdectoumax(
                        ...
                        b"\0" as *const u8 as *const libc::c_char,
                        gettext(
                            b"invalid input range\0" as *const u8 as *const libc::c_char,
                        ),
                        0 as libc::c_int,
                    );
                    *p = '-' as i32 as libc::c_char;
                    hi_optarg = p.offset(1 as libc::c_int as isize);
                }
                hi_input = xdectoumax(
                    hi_optarg,
                    0 as libc::c_int as uintmax_t,
    ...
    if echo as libc::c_int != 0 && input_range as libc::c_int != 0 {
        error(
            ...
            0 as libc::c_int,
            gettext(
                b"cannot combine -e and -i options\0" as *const u8 as *const libc::c_char,
            ),
        );
        usage(1 as libc::c_int);
    }
