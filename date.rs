...
pub const DEBUG_DATE_PARSING: C2RustUnnamed_1 = 129;
pub const RFC_3339_OPTION: C2RustUnnamed_1 = 128;
#[derive(Copy, Clone)]
#[repr(C)]
...
pub unsafe extern "C" fn usage(mut status: libc::c_int) -> ! {
    if status != 0 as libc::c_int {
        ...
        fputs_unlocked(
            gettext(
                b"Display the current time in the given FORMAT, or set the system date.\n\0"
                    as *const u8 as *const libc::c_char,
            ),
            stdout,
        );
...
unsafe extern "C" fn show_date(
    ...
    if parse_datetime_flags & 1 as libc::c_int as libc::c_uint != 0 {
        error(
            0 as libc::c_int,
            0 as libc::c_int,
            gettext(b"output format: %s\0" as *const u8 as *const libc::c_char),
            quote(format),
        );
    }
    if !(localtime_rz(tz, &mut when.tv_sec, &mut tm)).is_null() {
        ...
        error(
            0 as libc::c_int,
            0 as libc::c_int,
            gettext(b"time %s is out of range\0" as *const u8 as *const libc::c_char),
...
static mut long_options: [option; 16] = [
    ...
    },
    {
        let mut init = option {
            ...
            flag: 0 as *const libc::c_int as *mut libc::c_int,
            val: DEBUG_DATE_PARSING as libc::c_int,
        };
        init
    },
    {
    ...
    loop {
        ...
        if !parse_datetime2(
            ...
            error(
                0 as libc::c_int,
                0 as libc::c_int,
                gettext(b"invalid date %s\0" as *const u8 as *const libc::c_char),
                quote(line),
            );
...
unsafe fn main_0(
    ...
    let mut ok: bool = false;
    let mut option_specified_date: libc::c_int = 0;
    ...
    option_specified_date = (if !datestr.is_null() {
        1 as libc::c_int
    } else {
        0 as libc::c_int
    }) + (if !batch_file.is_null() { 1 as libc::c_int } else { 0 as libc::c_int })
        + (if !reference.is_null() { 1 as libc::c_int } else { 0 as libc::c_int });
    ...
    if optind < argc {
            ...
            as libc::c_int == '+' as i32
        {
            ...
            let fresh0 = optind;
            optind = optind + 1;
            format = (*argv.offset(fresh0 as isize)).offset(1 as libc::c_int as isize);
    ...
    if format.is_null() {
        ...
        if *format == 0 {
            format = b"%a %b %e %H:%M:%S %Z %Y\0" as *const u8 as *const libc::c_char;
        }
    }
        ...
        if set_date {
            if settime(&mut when) != 0 as libc::c_int {
                error(
                    0 as libc::c_int,
                    *__errno_location(),
                    gettext(b"cannot set date\0" as *const u8 as *const libc::c_char),
                );
                ok = 0 as libc::c_int != 0;
            }
        }
