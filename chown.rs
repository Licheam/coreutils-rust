...
extern "C" {
    ...
    fn uid_to_name(_: uid_t) -> *mut libc::c_char;
    fn chown_files(
...
pub const REFERENCE_FILE_OPTION: C2RustUnnamed_0 = 132;
pub const PRESERVE_ROOT: C2RustUnnamed_0 = 131;
pub const NO_PRESERVE_ROOT: C2RustUnnamed_0 = 130;
pub const FROM_OPTION: C2RustUnnamed_0 = 129;
pub const DEREFERENCE_OPTION: C2RustUnnamed_0 = 128;
#[derive(Copy, Clone)]
#[repr(C)]
...
pub unsafe extern "C" fn usage(mut status: libc::c_int) -> ! {
    if status != 0 as libc::c_int {
        ...
        printf(
            gettext(
                b"Usage: %s [OPTION]... [OWNER][:[GROUP]] FILE...\n  or:  %s [OPTION]... --reference=RFILE FILE...\n\0"
        ...
        fputs_unlocked(
            gettext(
                b"      --reference=RFILE  use RFILE's owner and group rather than\n                         specifying OWNER:GROUP values\n\0"
                    as *const u8 as *const libc::c_char,
            ),
            stdout,
        );
        ...
        emit_ancillary_info(b"chown\0" as *const u8 as *const libc::c_char);
    }
    exit(status);
}
#[inline]
...
unsafe fn main_0(
    ...
    let mut required_uid: uid_t = -(1 as libc::c_int) as uid_t;
    let mut required_gid: gid_t = -(1 as libc::c_int) as gid_t;
    ...
    loop {
        ...
        match optc {
            ...
            129 => {
                let mut e: *const libc::c_char = parse_user_spec(
                    optarg,
                    &mut required_uid,
                    &mut required_gid,
            ...
            -131 => {
                version_etc(
                    stdout,
                    b"chown\0" as *const u8 as *const libc::c_char,
                    b"GNU coreutils\0" as *const u8 as *const libc::c_char,
                    Version,
    ...
    if !reference_file.is_null() {
        ...
        let mut e_0: *const libc::c_char = parse_user_spec(
            *argv.offset(optind as isize),
            &mut uid,
            &mut gid,
            &mut chopt.user_name,
            &mut chopt.group_name,
        );
        if !e_0.is_null() {
        ...
        if (chopt.user_name).is_null() && !(chopt.group_name).is_null() {
            chopt.user_name = xstrdup(b"\0" as *const u8 as *const libc::c_char);
        }
        optind += 1;
        optind;
    }
