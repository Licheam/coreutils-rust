...
unsafe extern "C" fn print_full_info(mut username: *const libc::c_char) {
    ...
    printf(
        gettext(b"uid=%s\0" as *const u8 as *const libc::c_char),
    ...
    printf(
        gettext(b" gid=%s\0" as *const u8 as *const libc::c_char),
    ...
    if euid != ruid {
        printf(
            gettext(b" euid=%s\0" as *const u8 as *const libc::c_char),
    ...
    if egid != rgid {
        printf(
            gettext(b" egid=%s\0" as *const u8 as *const libc::c_char),
    ...
    if n_groups > 0 as libc::c_int {
        fputs_unlocked(
            gettext(b" groups=\0" as *const u8 as *const libc::c_char),
            stdout,
        );
    }
    ...
    if !context.is_null() {
        printf(gettext(b" context=%s\0" as *const u8 as *const libc::c_char), context);
    }
}
...
unsafe fn main_0(
    ...
    let mut selinux_enabled: libc::c_int = (is_selinux_enabled() > 0 as libc::c_int)
        as libc::c_int;
