...
pub const TTY_WRITE_ERROR: C2RustUnnamed_0 = 3;
pub const TTY_FAILURE: C2RustUnnamed_0 = 2;
pub const TTY_STDIN_NOTTY: C2RustUnnamed_0 = 1;
#[no_mangle]
pub unsafe extern "C" fn usage(mut status: libc::c_int) -> ! {
    if status != 0 as libc::c_int {
        ...
        emit_ancillary_info(b"tty\0" as *const u8 as *const libc::c_char);
    }
    exit(status);
}
#[inline]
...
unsafe fn main_0(
    ...
    initialize_exit_failure(TTY_WRITE_ERROR as libc::c_int);
    atexit(Some(close_stdout as unsafe extern "C" fn() -> ()));
    silent = 0 as libc::c_int != 0;
    loop {
        optc = getopt_long(
            argc,
            argv,
        ...
        match optc {
            ...
            _ => {
                usage(TTY_FAILURE as libc::c_int);
            }
        }
    }
    if optind < argc {
    ...
    *__errno_location() = 2 as libc::c_int;
    if silent {
        return if isatty(0 as libc::c_int) != 0 {
            0 as libc::c_int
        } else {
            TTY_STDIN_NOTTY as libc::c_int
        };
    }
    let mut status: libc::c_int = 0 as libc::c_int;
    let mut tty: *const libc::c_char = ttyname(0 as libc::c_int);
    if tty.is_null() {
        tty = gettext(b"not a tty\0" as *const u8 as *const libc::c_char);
        status = TTY_STDIN_NOTTY as libc::c_int;
    }
    puts(tty);
    return status;
}
