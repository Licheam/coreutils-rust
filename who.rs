...
pub const READ_UTMP_USER_PROCESS: C2RustUnnamed_1 = 2;
pub const READ_UTMP_CHECK_PIDS: C2RustUnnamed_1 = 1;
pub type C2RustUnnamed_2 = libc::c_uint;
pub const LOOKUP_OPTION: C2RustUnnamed_2 = 128;
#[derive(Copy, Clone)]
#[repr(C)]
...
unsafe extern "C" fn idle_string(
                ...
                as libc::c_long) < when && when <= now
    {
        ...
        if seconds_idle < 60 as libc::c_int {
            ...
            'c_10941: {
                if seconds_idle / (60 as libc::c_int * 60 as libc::c_int)
                    < 24 as libc::c_int
                {} else {
                    __assert_fail(
                        b"seconds_idle / (60 * 60) < 24\0" as *const u8
...
unsafe extern "C" fn make_id_equals_comment(
    ...
    let mut comment: *mut libc::c_char = xmalloc(
        (strlen(gettext(b"id=\0" as *const u8 as *const libc::c_char)))
            .wrapping_add(utmpsize)
            .wrapping_add(1 as libc::c_int as libc::c_ulong),
    ) as *mut libc::c_char;
    let mut p: *mut libc::c_char = stpcpy(
        comment,
        gettext(b"id=\0" as *const u8 as *const libc::c_char),
    );
...
unsafe extern "C" fn scan_entries(mut n: size_t, mut utmp_buf: *const STRUCT_UTMP) {
    ...
    loop {
            ...
            ) == 0 as libc::c_int
        {
            if need_users as libc::c_int != 0
                && ((*utmp_buf).ut_user[0 as libc::c_int as usize] as libc::c_int != 0
                    && ((*utmp_buf).ut_type as libc::c_int == 7 as libc::c_int
                        || 0 as libc::c_int != 0
                            && (*utmp_buf).ut_tv.tv_sec != 0 as libc::c_int))
            {
        ...
        if (*utmp_buf).ut_type as libc::c_int == 2 as libc::c_int {
            boottime = (*utmp_buf).ut_tv.tv_sec as time_t;
        }
        utmp_buf = utmp_buf.offset(1);
        utmp_buf;
    };
...
unsafe fn main_0(
    ...
    match argc - optind {
        2 => {
            my_line_only = 1 as libc::c_int != 0;
            current_block_70 = 16418587454608096141;
        }
        -1 | 0 => {
            current_block_70 = 16418587454608096141;
        }
        1 => {
            ...
            current_block_70 = 9437013279121998969;
        }
        _ => {
            error(
                0 as libc::c_int,
