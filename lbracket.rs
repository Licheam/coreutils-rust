...
unsafe fn main_0(
    ...
    {
        test_syntax_error(
            gettext(b"missing %s\0" as *const u8 as *const libc::c_char),
    ...
    if pos != argc {
        test_syntax_error(
            gettext(b"extra argument %s\0" as *const u8 as *const libc::c_char),
    ...
    match nargs {
        ...
        4 => {
                ...
                ) == 0 as libc::c_int
            {
                ...
                current_block_11 = 17872721378011645104;
            }
        }
        5 | _ => {
            current_block_11 = 17872721378011645104;
...
unsafe extern "C" fn term() -> bool {
            ...
            == '\0' as i32
    {
        ...
        if (*argv.offset(pos as isize)).is_null() {
            test_syntax_error(
                gettext(b"%s expected\0" as *const u8 as *const libc::c_char),
                quote(b")\0" as *const u8 as *const libc::c_char),
            );
        ...
        {
            test_syntax_error(
                gettext(b"%s expected, found %s\0" as *const u8 as *const libc::c_char),
...
unsafe extern "C" fn one_argument() -> bool {
    let fresh1 = pos;
    pos = pos + 1;
    return *(*argv.offset(fresh1 as isize)).offset(0 as libc::c_int as isize)
        as libc::c_int != '\0' as i32;
}
...
pub unsafe extern "C" fn usage(mut status: libc::c_int) -> ! {
    if status != 0 as libc::c_int {
        ...
        printf(
            ...
            gettext(b"test and/or [\0" as *const u8 as *const libc::c_char),
        );
...
unsafe extern "C" fn beyond() -> ! {
    test_syntax_error(
        gettext(b"missing argument after %s\0" as *const u8 as *const libc::c_char),
...
unsafe extern "C" fn find_int(mut string: *const libc::c_char) -> *const libc::c_char {
    ...
    let fresh2 = p;
    p = p.offset(1);
    if (*fresh2 as libc::c_uint).wrapping_sub('0' as i32 as libc::c_uint)
    ...
    test_syntax_error(
        gettext(b"invalid integer %s\0" as *const u8 as *const libc::c_char),
        quote(string),
    );
}
pub fn main() {
