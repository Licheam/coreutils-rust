...
pub const TOTAL_OPTION: C2RustUnnamed_1 = 131;
pub const OUTPUT_DELIMITER_OPTION: C2RustUnnamed_1 = 130;
pub const NOCHECK_ORDER_OPTION: C2RustUnnamed_1 = 129;
pub const CHECK_ORDER_OPTION: C2RustUnnamed_1 = 128;
#[derive(Copy, Clone)]
#[repr(C)]
        ...
        || !(thisline[1 as libc::c_int as usize]).is_null()
    {
        ...
        if (thisline[0 as libc::c_int as usize]).is_null() {
            ...
            if order == 0 as libc::c_int {
                order = if (*thisline[0 as libc::c_int as usize]).length
                    < (*thisline[1 as libc::c_int as usize]).length
                {
                    -(1 as libc::c_int)
                } else {
        ...
        if order == 0 as libc::c_int {
            ...
            total[2 as libc::c_int as usize];
