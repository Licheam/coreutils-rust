    ...
    while !tmp.is_null() {
        ...
        if g.is_null() {
            ...
            error(
                0 as libc::c_int,
                *__errno_location(),
                gettext(b"invalid group %s\0" as *const u8 as *const libc::c_char),
                quote(tmp),
            );
        } else {
    ...
    if ret == 0 as libc::c_int && n_gids == 0 as libc::c_int as libc::c_ulong {
        if show_errors {
            error(
                0 as libc::c_int,
                0 as libc::c_int,
                gettext(b"invalid group list %s\0" as *const u8 as *const libc::c_char),
                quote(groups),
            );
        }
        ret = -(1 as libc::c_int);
...
unsafe fn main_0(
    ...
    if !userspec.is_null() {
        let mut err: *const libc::c_char = parse_user_spec(
            userspec,
            &mut uid,
            &mut gid,
            0 as *mut *mut libc::c_char,
        ...
        if !err.is_null() && uid_unset(uid) as libc::c_int != 0
            && gid_unset(gid) as libc::c_int != 0
        {
    ...
    if !gid_unset(gid) && setgid(gid) != 0 {
        if ::core::mem::size_of::<C2RustUnnamed_5>() as libc::c_ulong != 0 {
            error(
                EXIT_CANCELED as libc::c_int,
                *__errno_location(),
                gettext(b"failed to set group-ID\0" as *const u8 as *const libc::c_char),
            );
            if 0 as libc::c_int != 0 {} else {
                unreachable!();
            };
    ...
    if !uid_unset(uid) && setuid(uid) != 0 {
        if ::core::mem::size_of::<C2RustUnnamed_4>() as libc::c_ulong != 0 {
            error(
                EXIT_CANCELED as libc::c_int,
                *__errno_location(),
                gettext(b"failed to set user-ID\0" as *const u8 as *const libc::c_char),
            );
            if 0 as libc::c_int != 0 {} else {
                unreachable!();
            };
    ...
    error(
        0 as libc::c_int,
        *__errno_location(),
        gettext(b"failed to run command %s\0" as *const u8 as *const libc::c_char),
