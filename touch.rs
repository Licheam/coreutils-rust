...
pub const TIME_OPTION: C2RustUnnamed_0 = 128;
#[derive(Copy, Clone)]
#[repr(C)]
...
unsafe extern "C" fn get_reldate(
    ...
    if !parse_datetime(result, flex_date, now) {
        if ::core::mem::size_of::<C2RustUnnamed_1>() as libc::c_ulong != 0 {
            error(
                1 as libc::c_int,
                0 as libc::c_int,
                gettext(b"invalid date format %s\0" as *const u8 as *const libc::c_char),
                quote(flex_date),
            );
    ...
    if change_times != 1 as libc::c_int | 2 as libc::c_int {
        if change_times == 2 as libc::c_int {
            ...
            'c_7490: {
                if change_times == 1 as libc::c_int {} else {
                    __assert_fail(
                        b"change_times == CH_ATIME\0" as *const u8
                            as *const libc::c_char,
                        b"src/touch.c\0" as *const u8 as *const libc::c_char,
                        147 as libc::c_int as libc::c_uint,
                        (*::core::mem::transmute::<
    ...
    if utime_errno != 0 as libc::c_int {
                        ...
                        == 0o40000 as libc::c_int as libc::c_uint)
        {
            error(
                0 as libc::c_int,
                open_errno,
                gettext(b"cannot touch %s\0" as *const u8 as *const libc::c_char),
                quotearg_style(shell_escape_always_quoting_style, file),
            );
        } else {
            ...
            error(
                0 as libc::c_int,
                utime_errno,
                gettext(b"setting times of %s\0" as *const u8 as *const libc::c_char),
                quotearg_style(shell_escape_always_quoting_style, file),
            );
        }
        return 0 as libc::c_int != 0;
    ...
    if use_ref {
        ...
        if !flex_date.is_null() {
            ...
            if change_times & 2 as libc::c_int != 0 {
                get_reldate(
                    &mut *newtime.as_mut_ptr().offset(1 as libc::c_int as isize),
                    flex_date,
                    &mut *newtime.as_mut_ptr().offset(1 as libc::c_int as isize),
                );
            }
            ...
            && newtime[0 as libc::c_int as usize].tv_nsec == now.tv_nsec
        {
            ...
            get_reldate(&mut notnow1, flex_date, &mut notnow);
