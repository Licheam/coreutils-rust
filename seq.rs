...
unsafe extern "C" fn scan_arg(mut arg: *const libc::c_char) -> operand {
    ...
    if ret.value != ret.value {
        error(
            0 as libc::c_int,
            0 as libc::c_int,
            gettext(b"invalid %s argument: %s\0" as *const u8 as *const libc::c_char),
            quote_n(
                0 as libc::c_int,
...
unsafe fn main_0(
        ...
        && strlen(separator) == 1 as libc::c_int as libc::c_ulong
    {
        ...
        if seq_fast(s1, s2, (step.value).to_u64().unwrap()) {
            return 0 as libc::c_int;
        }
    }
    let fresh4 = optind;
    optind = optind + 1;
    last = scan_arg(*argv.offset(fresh4 as isize));
    if optind < argc {
        first = last;
        let fresh5 = optind;
        optind = optind + 1;
        last = scan_arg(*argv.offset(fresh5 as isize));
        if optind < argc {
            step = last;
            ...
            let fresh6 = optind;
            optind = optind + 1;
            last = scan_arg(*argv.offset(fresh6 as isize));
        }
    }
        ...
        && format_str.is_null() && strlen(separator) == 1 as libc::c_int as libc::c_ulong
    {
        ...
        if *s1_0 as libc::c_int != '-' as i32 && *s2_0 as libc::c_int != '-' as i32
            && seq_fast(s1_0, s2_0, (step.value).to_u64().unwrap()) as libc::c_int != 0
        {
            return 0 as libc::c_int;
        }
