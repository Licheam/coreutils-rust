...
unsafe extern "C" fn tac_seekable(
    ...
    if remainder != 0 as libc::c_int as libc::c_ulong {
        ...
        if lseek(input_fd, file_pos, 0 as libc::c_int) < 0 as libc::c_int as libc::c_long
        {
            error(
                0 as libc::c_int,
                *__errno_location(),
                gettext(b"%s: seek failed\0" as *const u8 as *const libc::c_char),
    ...
    loop {
        if sentinel_length == 0 as libc::c_int as libc::c_ulong {
            ...
            let mut range: regoff_t = 1 as libc::c_int as libc::c_long - ri;
            let mut ret: regoff_t = 0;
            if (1 as libc::c_int as libc::c_long) < range {
            ...
            if range == 1 as libc::c_int as libc::c_long
                || {
                    ret = rpl_re_search(
            ...
            } else if ret == -(2 as libc::c_int) as libc::c_long {
    ...
    loop {
        ...
        ) != bytes_read
        {
            error(
                0 as libc::c_int,
                *__errno_location(),
                gettext(b"%s: write error\0" as *const u8 as *const libc::c_char),
                quotearg_n_style_colon(
                    0 as libc::c_int,
                    shell_escape_quoting_style,
                    file_name,
...
unsafe fn main_0(
    ...
    while !(*file.offset(i as isize)).is_null() {
        ok = (ok as libc::c_int & tac_file(*file.offset(i as isize)) as libc::c_int) != 0;
        i = i.wrapping_add(1);
        i;
    }
    output(
