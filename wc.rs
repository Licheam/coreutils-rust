...
extern "C" {
    ...
    fn wc_lines_avx2(
        ...
        fd: libc::c_int,
        lines_out: *mut uintmax_t,
        bytes_out: *mut uintmax_t,
    ) -> bool;
}
...
pub const GETOPT_VERSION_CHAR: C2RustUnnamed_4 = -131;
pub const GETOPT_HELP_CHAR: C2RustUnnamed_4 = -130;
#[derive(Copy, Clone)]
#[repr(C)]
pub struct infomap {
...
pub const FILES0_FROM_OPTION: C2RustUnnamed_5 = 129;
pub const DEBUG_PROGRAM_OPTION: C2RustUnnamed_5 = 128;
#[derive(Copy, Clone)]
#[repr(C)]
...
#[inline]
unsafe extern "C" fn __get_cpuid_count(
    mut __leaf: libc::c_uint,
    mut __subleaf: libc::c_uint,
    mut __eax: *mut libc::c_uint,
    mut __ebx: *mut libc::c_uint,
    mut __ecx: *mut libc::c_uint,
    mut __edx: *mut libc::c_uint,
    ...
    let fresh6 = &mut *__eax;
    let fresh7;
    let fresh8 = __leaf;
    let fresh9 = &mut *__ecx;
    let fresh10;
    let fresh11 = __subleaf;
    asm!(
        "  xchgq  %rbx,{1:r}\n  cpuid\n  xchgq  %rbx,{1:r}", inlateout("ax")
        c2rust_asm_casts::AsmCast::cast_in(fresh6, fresh8) => fresh7, lateout(reg) *
        __ebx, inlateout("cx") c2rust_asm_casts::AsmCast::cast_in(fresh9, fresh11) =>
        fresh10, lateout("dx") * __edx, options(preserves_flags, pure, readonly,
        att_syntax)
    );
...
unsafe extern "C" fn avx2_supported() -> bool {
    ...
    let mut getcpuid_ok: bool = 0 as libc::c_int != 0;
    let mut avx_enabled: bool = 0 as libc::c_int != 0;
    if __get_cpuid(
    ...
    {
        getcpuid_ok = 1 as libc::c_int != 0;
        if ecx & 0x8000000 as libc::c_int as libc::c_uint != 0 {
            avx_enabled = 1 as libc::c_int != 0;
        }
    }
    if avx_enabled {
        edx = 0 as libc::c_int as libc::c_uint;
        ecx = edx;
        ebx = ecx;
        eax = ebx;
        ...
        {
            getcpuid_ok = 0 as libc::c_int != 0;
        } else if ebx & 0x20 as libc::c_int as libc::c_uint == 0 {
            avx_enabled = 0 as libc::c_int != 0;
        }
    }
    if !getcpuid_ok {
        if debug {
        ...
        if debug {
            error(
                ...
                b"%s\0" as *const u8 as *const libc::c_char,
                gettext(
                    b"avx2 support not detected\0" as *const u8 as *const libc::c_char,
                ),
            );
        }
        return 0 as libc::c_int != 0;
        ...
        if debug {
            error(
                ...
                b"%s\0" as *const u8 as *const libc::c_char,
                gettext(
                    b"using avx2 hardware support\0" as *const u8 as *const libc::c_char,
                ),
            );
        }
        return 1 as libc::c_int != 0;
...
unsafe extern "C" fn wc(
        ...
        && !count_complicated
    {
        ...
        {
            let mut end_pos: size_t = (*fstatus).st.st_size as size_t;
            if current_pos < 0 as libc::c_int as libc::c_long {
                current_pos = lseek(fd, 0 as libc::c_int as __off_t, 1 as libc::c_int);
            }
            if end_pos.wrapping_rem(page_size) != 0 {
                bytes = if end_pos < current_pos as libc::c_ulong {
                    0 as libc::c_int as libc::c_ulong
                } else {
                    end_pos.wrapping_sub(current_pos as libc::c_ulong)
                };
                        ...
                        6545907279487748450 => {
                            match wide_char {
                                10 => {
                                    lines = lines.wrapping_add(1);
                                    lines;
                                    current_block = 17619485253189815129;
                                }
                                13 | 12 => {
                                    current_block = 17619485253189815129;
                                }
                                9 => {
                                    ...
                                    current_block = 10647240178681604119;
                                }
                                32 => {
                                ...
                                _ => {
                                    if wide as libc::c_int != 0
                                        && iswprint(wide_char as wint_t) != 0
                                    {
                                        if print_linelength {
                                        ...
                                        if iswspace(wide_char as wint_t) != 0
                                            || iswnbspace(wide_char as wint_t) != 0
                                        {
                                            current_block = 10647240178681604119;
                                        } else {
                ...
                loop {
                    ...
                    match *fresh13 as libc::c_int {
                        10 => {
                            lines = lines.wrapping_add(1);
                            lines;
                            current_block = 2167783540576651744;
                        }
                        13 | 12 => {
                            current_block = 2167783540576651744;
                        }
                        9 => {
                            ...
                            current_block = 2671786492858136600;
                        }
                        32 => {
...
unsafe fn main_0(
    ...
    if (1 as libc::c_int as libc::c_ulong) < argv_iter_n_args(ai) {
        write_counts(
            total_lines,
            total_words,
            total_chars,
