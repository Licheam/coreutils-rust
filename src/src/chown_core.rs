...
pub unsafe extern "C" fn uid_to_name(mut uid: uid_t) -> *mut libc::c_char {
    ...
    return xstrdup(
        if !pwd.is_null() {
            (*pwd).pw_name
        } else if !((0 as libc::c_int as uid_t) < -(1 as libc::c_int) as uid_t) {
...
pub unsafe extern "C" fn gid_to_name(mut gid: gid_t) -> *mut libc::c_char {
    ...
    return xstrdup(
        if !grp.is_null() {
            (*grp).gr_name
        } else if !((0 as libc::c_int as gid_t) < -(1 as libc::c_int) as gid_t) {
...
unsafe extern "C" fn restricted_chown(
        ...
        == 0o100000 as libc::c_int as libc::c_uint)
    {
        ...
        {
            open_flags |= 0o200000 as libc::c_int;
        } else {
            return RC_do_ordinary_chown
        }
    }
            ...
            error(
                ...
                *__errno_location(),
                if uid != -(1 as libc::c_int) as uid_t {
                    gettext(
                        b"changing ownership of %s\0" as *const u8 as *const libc::c_char,
                    )
                } else {
                    gettext(
