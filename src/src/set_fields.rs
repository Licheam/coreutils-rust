    ...
    if n_frp == 0 {
        error(
            ...
            if options & SETFLD_ERRMSG_USE_POS as libc::c_int as libc::c_uint != 0 {
                ...
                gettext(b"missing list of fields\0" as *const u8 as *const libc::c_char)
            },
        );
        usage(1 as libc::c_int);
    }
...
unsafe extern "C" fn compare_ranges(
    ...
    let mut a_start: libc::c_int = (*(a as *const field_range_pair)).lo as libc::c_int;
    let mut b_start: libc::c_int = (*(b as *const field_range_pair)).lo as libc::c_int;
    return if a_start < b_start {
        -(1 as libc::c_int)
    } else {
        (a_start > b_start) as libc::c_int
    };
}
