...
#[no_mangle]
pub unsafe extern "C" fn ftoastr(
    ...
    let mut promoted_x: libc::c_double = x as libc::c_double;
    let mut format: [libc::c_char; 11] = [0; 11];
    let mut abs_x: libc::c_float = if x < 0 as libc::c_int as libc::c_float {
        -x
    } else {
        x
    };
    ...
    *p = 'L' as i32 as libc::c_char;
    p = p.offset(((2 as libc::c_int) < 1 as libc::c_int) as libc::c_int as isize);
    let fresh4 = p;
    p = p.offset(1);
    ...
    *p = '\0' as i32 as libc::c_char;
    prec = if abs_x < 1.17549435e-38f32 { 1 as libc::c_int } else { 6 as libc::c_int };
    loop {
        ...
        if n < 0 as libc::c_int
            ...
            || (n as libc::c_ulong) < bufsize
                && strtof(buf, 0 as *mut *mut libc::c_char) == x
        {
            return n;
        }
        prec += 1;
