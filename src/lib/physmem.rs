...
#[no_mangle]
pub unsafe extern "C" fn physmem_total() -> libc::c_double {
    let mut pages: libc::c_double = sysconf(_SC_PHYS_PAGES as libc::c_int)
        as libc::c_double;
    let mut pagesize: libc::c_double = sysconf(_SC_PAGESIZE as libc::c_int)
        as libc::c_double;
    if 0 as libc::c_int as libc::c_double <= pages
        && 0 as libc::c_int as libc::c_double <= pagesize
    {
        return pages * pagesize;
    }
    let mut si: sysinfo = sysinfo {
        uptime: 0,
        loads: [0; 3],
        totalram: 0,
        freeram: 0,
    ...
    if sysinfo(&mut si) == 0 as libc::c_int {
        return si.totalram as libc::c_double * si.mem_unit as libc::c_double;
    }
    return (64 as libc::c_int * 1024 as libc::c_int * 1024 as libc::c_int)
        as libc::c_double;
}
#[no_mangle]
pub unsafe extern "C" fn physmem_available() -> libc::c_double {
    let mut pages: libc::c_double = sysconf(_SC_AVPHYS_PAGES as libc::c_int)
        as libc::c_double;
    let mut pagesize: libc::c_double = sysconf(_SC_PAGESIZE as libc::c_int)
        as libc::c_double;
    if 0 as libc::c_int as libc::c_double <= pages
    ...
    if sysinfo(&mut si) == 0 as libc::c_int {
        return (si.freeram as libc::c_double + si.bufferram as libc::c_double)
            * si.mem_unit as libc::c_double;
    }
    return physmem_total() / 4 as libc::c_int as libc::c_double;
}
