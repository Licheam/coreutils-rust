...
#[no_mangle]
pub unsafe extern "C" fn ldtoastr(
    ...
    let mut promoted_x: f128::f128 = x;
    let mut format: [libc::c_char; 11] = [0; 11];
    let mut abs_x: f128::f128 = if x < f128::f128::new(0 as libc::c_int) {
        -x
    } else {
        x
    };
    ...
    *p = 'L' as i32 as libc::c_char;
    p = p.offset(((2 as libc::c_int) < 3 as libc::c_int) as libc::c_int as isize);
    let fresh4 = p;
    p = p.offset(1);
    ...
    *p = '\0' as i32 as libc::c_char;
    prec = if abs_x < f128::f128::new(3.36210314311209350626e-4932) {
        1 as libc::c_int
    } else {
        18 as libc::c_int
    };
    loop {
        ...
        if n < 0 as libc::c_int
            ...
            || (n as libc::c_ulong) < bufsize
                && strtold(buf, 0 as *mut *mut libc::c_char) == x
        {
            return n;
        }
        prec += 1;
