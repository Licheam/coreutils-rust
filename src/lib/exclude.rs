...
pub unsafe extern "C" fn free_exclude(mut ex: *mut exclude) {
    ...
    while !pbuf.is_null() {
        ...
        free((*pbuf).base as *mut libc::c_void);
        free(pbuf as *mut libc::c_void);
        pbuf = next_0;
    }
    free(ex as *mut libc::c_void);
}
#[no_mangle]
...
pub unsafe extern "C" fn add_exclude(
        ...
        && fnmatch_pattern_has_wildcards(pattern, options) as libc::c_int != 0
    {
        ...
        let fresh1 = (*pat).exclude_count;
        (*pat).exclude_count = (*pat).exclude_count + 1;
        patopts = &mut *((*pat).exclude).offset(fresh1 as isize) as *mut patopts;
...
pub unsafe extern "C" fn add_exclude_file(
    ...
    if !use_stdin && rpl_fclose(in_0) != 0 as libc::c_int {
        rc = -(1 as libc::c_int);
    }
    return rc;
}
...
pub unsafe extern "C" fn add_exclude_fp(
    ...
    loop {
        ...
        let fresh2 = buf_count;
        buf_count = buf_count + 1;
        *buf.offset(fresh2 as isize) = c as libc::c_char;
    }
    if ferror(fp) != 0 {
        e = *__errno_location();
    }
...
unsafe extern "C" fn mbuiter_multi_next(mut iter: *mut mbuiter_multi) {
    ...
    if (*iter).in_shift {
        current_block = 11538494623230309621;
...
unsafe extern "C" fn free_exclude_segment(mut seg: *mut exclude_segment) {
    match (*seg).type_0 as libc::c_uint {
        1 => {
            ...
            free((*seg).v.pat.exclude as *mut libc::c_void);
        }
        0 => {
