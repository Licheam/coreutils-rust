...
#[no_mangle]
pub unsafe extern "C" fn num_processors(mut query: nproc_query) -> libc::c_ulong {
    let mut omp_env_limit: libc::c_ulong = (9223372036854775807 as libc::c_long
        as libc::c_ulong)
        .wrapping_mul(2 as libc::c_ulong)
        .wrapping_add(1 as libc::c_ulong);
    if query as libc::c_uint == NPROC_CURRENT_OVERRIDABLE as libc::c_int as libc::c_uint
    {
        let mut omp_env_threads: libc::c_ulong = 0;
        omp_env_threads = parse_omp_threads(
            getenv(b"OMP_NUM_THREADS\0" as *const u8 as *const libc::c_char),
        );
        omp_env_limit = parse_omp_threads(
            getenv(b"OMP_THREAD_LIMIT\0" as *const u8 as *const libc::c_char),
        );
        if omp_env_limit == 0 {
            omp_env_limit = (9223372036854775807 as libc::c_long as libc::c_ulong)
                .wrapping_mul(2 as libc::c_ulong)
                .wrapping_add(1 as libc::c_ulong);
        }
        if omp_env_threads != 0 {
            return if omp_env_threads < omp_env_limit {
                omp_env_threads
            } else {
                omp_env_limit
            };
    ...
    let mut nprocs: libc::c_ulong = num_processors_ignoring_omp(query);
    return if nprocs < omp_env_limit { nprocs } else { omp_env_limit };
}
#[inline]
...
unsafe extern "C" fn num_processors_via_affinity_mask() -> libc::c_ulong {
...
unsafe extern "C" fn num_processors_ignoring_omp(
    mut query: nproc_query,
) -> libc::c_ulong {
    if query as libc::c_uint == NPROC_CURRENT as libc::c_int as libc::c_uint {
        let mut nprocs: libc::c_ulong = num_processors_via_affinity_mask();
        if nprocs > 0 as libc::c_int as libc::c_ulong {
            return nprocs;
        }
        let mut nprocs_0: libc::c_long = sysconf(_SC_NPROCESSORS_ONLN as libc::c_int);
        if nprocs_0 > 0 as libc::c_int as libc::c_long {
            return nprocs_0 as libc::c_ulong;
        }
    } else {
        let mut nprocs_1: libc::c_long = sysconf(_SC_NPROCESSORS_CONF as libc::c_int);
        if nprocs_1 == 1 as libc::c_int as libc::c_long
            || nprocs_1 == 2 as libc::c_int as libc::c_long
        {
            let mut nprocs_current: libc::c_ulong = num_processors_via_affinity_mask();
            if nprocs_current > nprocs_1 as libc::c_ulong {
                nprocs_1 = nprocs_current as libc::c_long;
            }
        }
        if nprocs_1 > 0 as libc::c_int as libc::c_long {
            return nprocs_1 as libc::c_ulong;
        }
    }
    return 1 as libc::c_int as libc::c_ulong;
}
unsafe extern "C" fn parse_omp_threads(
    mut threads: *const libc::c_char,
) -> libc::c_ulong {
    let mut ret: libc::c_ulong = 0 as libc::c_int as libc::c_ulong;
    if threads.is_null() {
        return ret;
    }
    while *threads as libc::c_int != '\0' as i32
        && c_isspace(*threads as libc::c_int) as libc::c_int != 0
    {
        threads = threads.offset(1);
        threads;
    }
    if c_isdigit(*threads as libc::c_int) {
        let mut endptr: *mut libc::c_char = 0 as *mut libc::c_char;
        let mut value: libc::c_ulong = strtoul(threads, &mut endptr, 10 as libc::c_int);
        if !endptr.is_null() {
            while *endptr as libc::c_int != '\0' as i32
                && c_isspace(*endptr as libc::c_int) as libc::c_int != 0
            {
            ...
            if *endptr as libc::c_int == '\0' as i32 {
                return value
            } else if *endptr as libc::c_int == ',' as i32 {
                return value
            }
        }
    }
