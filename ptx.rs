...
extern "C" {
    ...
    fn rpl_re_compile_fastmap(__buffer: *mut re_pattern_buffer) -> libc::c_int;
    fn rpl_re_search(
        __buffer: *mut re_pattern_buffer,
...
unsafe extern "C" fn copy_unescaped_string(
    ...
    result = xmalloc((strlen(string)).wrapping_add(1 as libc::c_int as libc::c_ulong))
        as *mut libc::c_char;
    cursor = result;
    while *string != 0 {
        if *string as libc::c_int == '\\' as i32 {
            ...
            match *string as libc::c_int {
                ...
                97 => {
                    let fresh4 = cursor;
                    cursor = cursor.offset(1);
                    *fresh4 = '\u{7}' as i32 as libc::c_char;
                    string = string.offset(1);
                    string;
                }
                98 => {
                    let fresh5 = cursor;
                    cursor = cursor.offset(1);
                    *fresh5 = '\u{8}' as i32 as libc::c_char;
                    string = string.offset(1);
                    string;
                }
                99 => {
                ...
                102 => {
                    let fresh6 = cursor;
                    cursor = cursor.offset(1);
                    *fresh6 = '\u{c}' as i32 as libc::c_char;
                    string = string.offset(1);
                    string;
                }
                110 => {
                ...
                118 => {
                    let fresh10 = cursor;
                    cursor = cursor.offset(1);
                    *fresh10 = '\u{b}' as i32 as libc::c_char;
                    string = string.offset(1);
                    string;
                }
                0 => {}
    ...
    if !message.is_null() {
        if ::core::mem::size_of::<C2RustUnnamed_2>() as libc::c_ulong != 0 {
            error(
                1 as libc::c_int,
                0 as libc::c_int,
                gettext(b"%s (for regexp %s)\0" as *const u8 as *const libc::c_char),
                message,
                quote(string),
            );
            unreachable!();
...
unsafe extern "C" fn initialize_regex() {
    ...
    if ignore_case {
        ...
        while character < 256 as libc::c_int {
            folded_chars[character as usize] = toupper(character) as libc::c_uchar;
            character += 1;
            character;
        }
    }
...
unsafe extern "C" fn compare_words(
    ...
    return if (*(void_first as *const WORD)).size < (*(void_second as *const WORD)).size
    {
        -(1 as libc::c_int)
    } else {
...
unsafe extern "C" fn compare_occurs(
    ...
    return if value != 0 {
        value
    } else if (*(void_first as *const OCCURS)).key.start
        < (*(void_second as *const OCCURS)).key.start
    {
        -(1 as libc::c_int)
    } else {
...
unsafe extern "C" fn digest_word_file(
    ...
    while cursor < file_contents.end {
        ...
        if cursor > word_start {
            ...
            let ref mut fresh16 = (*((*table).start).offset((*table).length as isize))
                .start;
            *fresh16 = word_start;
    ...
    while cursor < (*text_buffer).end {
        ...
        if !(context_regex.string).is_null() {
            ...
            match rpl_re_search(
                ...
                cursor,
                ((*text_buffer).end).offset_from(cursor) as libc::c_long,
                ...
                -1 => {
                    current_block_15 = 12147880666119273379;
                }
                0 => {
                    ...
                    current_block_15 = 4635498567638269552;
                }
                _ => {
                    current_block_15 = 4635498567638269552;
                }
        ...
        loop {
            if !(word_regex.string).is_null() {
                let mut r: regoff_t = rpl_re_search(
                    ...
                    cursor,
                    context_end.offset_from(cursor) as libc::c_long,
                    0 as libc::c_int as regoff_t,
                    context_end.offset_from(cursor) as libc::c_long,
                    &mut word_regs,
                );
            ...
            if word_end == word_start {
                ...
                number_of_occurs[0 as libc::c_int as usize];
            }
        }
...
unsafe extern "C" fn define_all_fields(mut occurs: *mut OCCURS) {
        ...
        && cursor <= (keyafter.start).offset(keyafter_max_width as isize)
    {
        ...
        if !(word_regex.string).is_null() {
            ...
            count = rpl_re_match(
                &mut word_regex.pattern,
                cursor,
                right_context_end.offset_from(cursor) as libc::c_long,
                0 as libc::c_int as regoff_t,
                0 as *mut re_registers,
            );
            if count == -(2 as libc::c_int) as libc::c_long {
                matcher_error();
            }
            cursor = cursor
                .offset(
    ...
    if -(*occurs).left > half_line_width + maximum_word_length {
        ...
        if !(word_regex.string).is_null() {
            ...
            if count_0 == -(2 as libc::c_int) as libc::c_long {
                matcher_error();
            }
            left_field_start = left_field_start
                .offset(
                    (if count_0 == -(1 as libc::c_int) as libc::c_long {
                        1 as libc::c_int as libc::c_long
                    } else {
                        count_0
                    }) as isize,
    ...
    while (before.start).offset(before_max_width as isize) < before.end {
        if !(word_regex.string).is_null() {
            ...
            if count_1 == -(2 as libc::c_int) as libc::c_long {
                matcher_error();
            }
            before
                .start = (before.start)
                .offset(
                    (if count_1 == -(1 as libc::c_int) as libc::c_long {
                        1 as libc::c_int as libc::c_long
                    } else {
                        count_1
                    }) as isize,
    ...
    if tail_max_width > 0 as libc::c_int as libc::c_long {
            ...
            && cursor < (tail.start).offset(tail_max_width as isize)
        {
            ...
            if !(word_regex.string).is_null() {
                ...
                if count_2 == -(2 as libc::c_int) as libc::c_long {
                    matcher_error();
                }
                cursor = cursor
                    .offset(
                        (if count_2 == -(1 as libc::c_int) as libc::c_long {
                            1 as libc::c_int as libc::c_long
                        } else {
                            count_2
                        }) as isize,
    ...
    if head_max_width > 0 as libc::c_int as libc::c_long {
        ...
        while (head.start).offset(head_max_width as isize) < head.end {
            if !(word_regex.string).is_null() {
                ...
                if count_3 == -(2 as libc::c_int) as libc::c_long {
                    matcher_error();
                }
                head
                    .start = (head.start)
                    .offset(
                        (if count_3 == -(1 as libc::c_int) as libc::c_long {
                            1 as libc::c_int as libc::c_long
                        } else {
                            count_3
                        }) as isize,
...
unsafe extern "C" fn output_one_tex_line() {
    ...
    if !(word_regex.string).is_null() {
        ...
        count = rpl_re_match(
            &mut word_regex.pattern,
            cursor,
            (keyafter.end).offset_from(cursor) as libc::c_long,
            0 as libc::c_int as regoff_t,
            0 as *mut re_registers,
        );
        if count == -(2 as libc::c_int) as libc::c_long {
...
unsafe fn main_0(
    ...
    if optind == argc {
        ...
        let ref mut fresh17 = *input_file_name.offset(0 as libc::c_int as isize);
        *fresh17 = 0 as *const libc::c_char;
        ...
        while file_index < number_input_files {
            ...
            {
                let ref mut fresh18 = *input_file_name.offset(file_index as isize);
                *fresh18 = 0 as *const libc::c_char;
            } else {
                let ref mut fresh19 = *input_file_name.offset(file_index as isize);
                *fresh19 = *argv.offset(optind as isize);
            }
            optind += 1;
            optind;
            file_index += 1;
        ...
        {
            let ref mut fresh20 = *input_file_name.offset(0 as libc::c_int as isize);
            *fresh20 = 0 as *const libc::c_char;
        } else {
            let ref mut fresh21 = *input_file_name.offset(0 as libc::c_int as isize);
            *fresh21 = *argv.offset(optind as isize);
        }
        optind += 1;
        optind;
        if optind < argc {
