...
pub const REFERENCE_FILE_OPTION: C2RustUnnamed_0 = 131;
pub const PRESERVE_ROOT: C2RustUnnamed_0 = 130;
pub const NO_PRESERVE_ROOT: C2RustUnnamed_0 = 129;
...
pub unsafe extern "C" fn usage(mut status: libc::c_int) -> ! {
    if status != 0 as libc::c_int {
        ...
        fputs_unlocked(
            gettext(
                b"      --reference=RFILE  use RFILE's group rather than specifying a\n                         GROUP value\n\0"
                    as *const u8 as *const libc::c_char,
            ),
            stdout,
        );
