...
pub const UNLINK_DEST_BEFORE_OPENING: C2RustUnnamed_0 = 136;
pub const STRIP_TRAILING_SLASHES_OPTION: C2RustUnnamed_0 = 135;
pub const SPARSE_OPTION: C2RustUnnamed_0 = 134;
pub const REFLINK_OPTION: C2RustUnnamed_0 = 133;
pub const PRESERVE_ATTRIBUTES_OPTION: C2RustUnnamed_0 = 132;
pub const PARENTS_OPTION: C2RustUnnamed_0 = 131;
pub const NO_PRESERVE_ATTRIBUTES_OPTION: C2RustUnnamed_0 = 130;
pub const COPY_CONTENTS_OPTION: C2RustUnnamed_0 = 129;
pub const ATTRIBUTES_ONLY_OPTION: C2RustUnnamed_0 = 128;
#[derive(Copy, Clone)]
#[repr(C)]
...
static mut long_opts: [option; 30] = [
    {
        let mut init = option {
    ...
    while !p.is_null() {
        *dst_name.offset((*p).slash_offset as isize) = '\0' as i32 as libc::c_char;
        if (*x).preserve_timestamps {
            ...
            if utimens(dst_name, timespec.as_mut_ptr() as *const timespec) != 0 {
                error(
                    0 as libc::c_int,
                    *__errno_location(),
                    gettext(
        ...
        if (*x).preserve_ownership {
            if lchown(dst_name, (*p).st.st_uid, (*p).st.st_gid) != 0 as libc::c_int {
                ...
                lchown(dst_name, -(1 as libc::c_int) as __uid_t, (*p).st.st_gid);
            }
        }
        if (*x).preserve_mode {
            if copy_acl(
            ...
            if lchmod(dst_name, (*p).st.st_mode) != 0 as libc::c_int {
                error(
                    0 as libc::c_int,
                    *__errno_location(),
                    gettext(
...
unsafe extern "C" fn make_dir_parents_private(
    ...
    let mut fresh2 = ::std::vec::from_elem(
        0,
        dirlen.wrapping_add(1 as libc::c_int as libc::c_ulong) as usize,
    );
    dst_dir = fresh2.as_mut_ptr() as *mut libc::c_char;
    memcpy(dst_dir as *mut libc::c_void, dir as *const libc::c_void, dirlen);
    *dst_dir.offset(dirlen as isize) = '\0' as i32 as libc::c_char;
    *attr_list = 0 as *mut dir_attr;
    if stat(dst_dir, &mut stats) != 0 as libc::c_int {
        ...
        loop {
            ...
            *slash = '\0' as i32 as libc::c_char;
            missing_dir = stat(dir, &mut stats) != 0 as libc::c_int;
            ...
            if missing_dir {
                ...
                *new_dst = 1 as libc::c_int != 0;
                ...
                if mkdir(dir, mkdir_mode) != 0 as libc::c_int {
                    error(
                        0 as libc::c_int,
                        *__errno_location(),
                        gettext(
...
unsafe extern "C" fn target_directory_operand(
    mut file: *const libc::c_char,
    mut st: *mut stat,
    mut new_dst: *mut bool,
    mut forcing: bool,
) -> bool {
    let mut err: libc::c_int = if stat(file, st) == 0 as libc::c_int {
        0 as libc::c_int
    } else {
        *__errno_location()
    };
...
unsafe extern "C" fn do_copy(
    ...
    let mut forcing: bool = (*x).unlink_dest_before_opening as libc::c_int != 0
    ...
    if !target_directory.is_null() {
            ...
            if !parent_exists {
                ...
                if parents_option {
                    ok = (ok as libc::c_int
                        & re_protect(
                            dst_name,
                            arg_in_concat.offset_from(dst_name) as libc::c_long
                                as size_t,
                            attr_list,
                            x,
                        ) as libc::c_int) as bool;
        ...
        let mut new_dest: *const libc::c_char = 0 as *const libc::c_char;
...
unsafe fn main_0(
    ...
    selinux_enabled = (0 as libc::c_int) < is_selinux_enabled();
    ...
    loop {
        ...
        match c {
            134 => {
                ...
                current_block_81 = 16789764818708874114;
            }
            133 => {
                if optarg.is_null() {
            ...
            132 => {
                if optarg.is_null() {
                    current_block_81 = 7175012969253723235;
                } else {
            ...
            116 => {
                if !target_directory.is_null() {
                    ...
                    if stat(optarg, &mut st) != 0 as libc::c_int {
                        if ::core::mem::size_of::<C2RustUnnamed_9>() as libc::c_ulong
                            != 0
                        {
                            error(
                                1 as libc::c_int,
                    ...
                    if !(st.st_mode & 0o170000 as libc::c_int as libc::c_uint
                        == 0o40000 as libc::c_int as libc::c_uint)
                    {
                        if ::core::mem::size_of::<C2RustUnnamed_8>() as libc::c_ulong
                            != 0
                        {
                            error(
                                1 as libc::c_int,
